...
<script type="text/javascript">
	...
	function doAssociations(){
		...
		if(rows.length != 1){
			//弹出提示
			$.messager.alert("提示信息","请选择一个承包区操作！","warning");
			return;
		}else{
			//选中了一个定区
			...
			var url_1 = "crm/findCustomersNoAss";
			...
			//发送ajax请求，请求定区Action，在定区Action中通过crm代理对象完成对于crm服务远程调用获取客户数据
			var url_2 = "crm/findCustomersByDzoneid";
			var decidedzoneId = rows[0].id;
			$.post(url_2,{"dzid":decidedzoneId},function(data){
				//遍历json数组
				for(var i=0;i<data.length;i++){
					var id = data[i].id;
					var name = data[i].name;
	...
	$(function(){
		...
		$('#grid').datagrid( {
			...
			pagination : true,
			toolbar : toolbar,
			url : "static/json/decidedzone.json",
			idField : 'id',
			columns : columns,
